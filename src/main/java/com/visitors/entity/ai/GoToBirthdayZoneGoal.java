...
/**
 * Goal de IA para que los visitantes de cumpleaños vayan primero a la zona
 * principal y luego a la zona de cumpleaños.
 */
public class GoToBirthdayZoneGoal extends Goal {
    ...
    private boolean reachedBirthdayArea = false;
    private static final int MAX_TICKS = 2400; // 2 minutes max

    public GoToBirthdayZoneGoal(VisitorEntity visitor, double speed) {
    ...
    public boolean canUse() {
        return visitor.isBirthdayParty() &&
                visitor.getVisitorState() == VisitorEntity.VisitorState.GOING_TO_BIRTHDAY;
    }

    @Override
    public boolean canContinueToUse() {
        return canUse() && !reachedBirthdayArea && ticksMoving < MAX_TICKS;
    }
    ...
    public void stop() {
        targetPos = null;
        if (reachedBirthdayArea) {
            // Stay wandering in birthday area
    ...
    private void checkProgress() {
        ...
        // Check if in birthday area
        if (reachedMainArea && data.hasValidBirthdayZone()) {
            ...
            AABB birthdayBounds = new AABB(min, max.offset(1, 1, 1));
            if (birthdayBounds.contains(visitor.position())) {
                reachedBirthdayArea = true;
            }
        }
    }
    ...
    private void findTargetPosition() {
        ...
        } else if (!reachedBirthdayArea && data.hasValidBirthdayZone()) {
            // Then go to birthday zone
            BlockPos bp1 = data.getBirthdayPos1();
            BlockPos bp2 = data.getBirthdayPos2();
